...
bool Assets::LvglStrategy::InitializePartition(Assets* assets) {
    ...
    // --- DIAGNOSTIC: independently re-read the same range via esp_partition_read
    // (bypassing the mmap/cache path entirely) and recompute the checksum from
    // that separately-obtained buffer, to determine whether mmap is returning
    // stale/incorrect data versus the checksum math itself being wrong.
    ESP_LOGI(TAG, "DIAG: stored_files=%lu stored_chksum=0x%lx stored_len=%lu map_size=%lu partition_size=%lu",
             (unsigned long)stored_files, (unsigned long)stored_chksum, (unsigned long)stored_len,
